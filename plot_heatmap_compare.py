...
for nutrient in nutrient_total_keys:
    name = nutrient.replace('2014 Total', '') + ' per 1kL'   #.replace('(Kg)', '').replace('(g)', '').replace('(mg)', '')
...
fig.tight_layout()
plt.savefig('figures/heatmap_compare_water_efficiency.png', bbox_inches='tight', dpi=200, transparent=False)
# plt.show()